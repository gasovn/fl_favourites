...
function migrate(storage, callback = noop, errorCallback = noop) {
  storage.get(defaultOptions, function(data) {
    switch (data.storage_schema) {
    case 0: // Rename saved array in storage
      storage.get(['branch_faves', 'branch_fave_array'], (moreData) => {
        if (moreData.branch_faves) {
          data.branch_fave_array = moreData.branch_faves;
          data.storage_schema = 1;
          storage.set(data, () => {
            migrate(storage, callback);
          });
        }
      });
      break;
    case 1: // Migrate from saved arrays to packed sets
      ...
      break;
    case defaultOptions.storage_schema: // Expected; we're at current version
    ...
    default: // Unknown version - probably synced from newer one
      console.error(`Unknown data storage schema (got ${data.storage_schema},` +
          `expected ${defaultOptions.storage_schema})`);
